...
def tablecreation():
    ...
    try:
        ...
        drop_command = "DROP TABLE IF EXISTS habit_db"
...
def predefinedhabits():
    ...
    select_all = "SELECT * FROM habit_db"
    habit_list = conn.execute(select_all)
    ...
    conn.executemany(
        "INSERT OR REPLACE INTO habit_db ('Title','Period','Creation','Start_Date','Due_Date','Streak','Max_Streak','Break') VALUES (?,?,?,?,?,?,?,?)",
        habits)
